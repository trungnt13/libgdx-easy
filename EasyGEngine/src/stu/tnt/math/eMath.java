...
public final class eMath {
	...
	public static float calVectorAngle(float x, float y, float x1, float y1) {
		...
		if (x1 > x)
			return (float) (180.0f / MathUtils.PI * Math.atan((y1 - y)
					/ (x1 - x)));
		if (x1 < x)
			return (float) (180.0f / MathUtils.PI * Math.atan((y1 - y)
					/ (x1 - x))) - 180;
	...
	public static float calModule(Vector2 src, Vector2 dst) {
		return (float) Math.sqrt((src.x - dst.x) * (src.x - dst.x)
	...
	public static void calCenterPoint(Vector2 result, float x, float y,
			float x1, float y1) {
	 ...
	 */
	public static float calAngle(float x, float y, float x1, float y1,
			float x2, float y2) {
		float a;
		float b;
		float c;
	...
	public static Vector2 solveTheQuadricFunction(float a, float b, float c) {
		if (a == 0)
			...
			return new Vector2((float) (-b + Math.sqrt(b * b - 4 * a * c))
					/ (2 * a), (float) (-b - Math.sqrt(b * b - 4 * a * c))
					/ (2 * a));
		}
	 ...
	 * @return
	 */
	public static int calShortestPathBetweenTwoAngle(float srcANGLE,
			float dstANGLE) {
		if (dstANGLE <= 180) {
			if (srcANGLE > 180) {
	...
	/**
	 * This method is more special if your number is power of two it will return
	 * a number >= 0 ,otherwise it will return < 0
	 * 
	 * @param number
	 * @return
	 */
