...
public class Line implements Disposable {
	...
	public Line(Vector2 startPoint, Line line, byte linemode) {
		if (linemode == LineMode.SQUARE_LINE) {
			...
			Vector2 endPoint = new Vector2(startPoint.x + d.x, startPoint.y
					+ d.y);
	...
	public float getAngleFactor() {
		...
		if (lineEnd.x > lineStart.x)
			return (float) (180.0f / MathUtils.PI * Math
					.atan((lineEnd.y - lineStart.y) / (lineEnd.x - lineStart.x)));
		if (lineEnd.x < lineStart.x)
			return (float) (180.0f / MathUtils.PI * Math
					.atan((lineEnd.y - lineStart.y) / (lineEnd.x - lineStart.x))) - 180;
	...
	public float module() {
		float tmp = (float) Math.sqrt((lineStart.x - lineEnd.x)
	...
	public float module2() {
		float tmp = (float) ((lineStart.x - lineEnd.x)
				* (lineStart.x - lineEnd.x) + (lineStart.y - lineEnd.y)
				* (lineStart.y - lineEnd.y));
		return tmp;
	}
	...
	public Vector2 calCrossPoint(Line line) {
		...
		if (a1 * b2 == b1 * a2)
			...
			return new Vector2((b1 * c2 - c1 * b2) / (b2 * a1 - b1 * a2), (a2
					* c1 - c2 * a1)
					/ (b2 * a1 - b1 * a2));
	}
