...
public abstract class GameCore implements ApplicationListener {
	...
	public void resume() {
		...
		if (!isStarted) {
			screen = mStartScreen;
			mStartScreen
					.setChangedScreenListener(new LoadingScreen.OnChangedScreen() {
						@Override
			...
			mStartScreen.show();
			mStartScreen.resize(Gdx.graphics.getWidth(),
	...
	protected void setStartScreen(LoadingScreen loadScreen) {
		if (isStarted)
			throw new EasyGEngineRuntimeException(
					"One GameCore cant set start screen twice");
	...
	void setScreen(Screen screen, int destroyMode) {
		 ...
		 * screen, the show method of next screen only call for the first time
		 * load
		 */
		isLoading = (this.screen instanceof LoadingScreen);
		if (isLoading)
			isLoading = isLoading
					& !((LoadingScreen) this.screen).isFirstTimeLoad();
	...
	void setScreen(Screen screen, ResourceContext... contexts) {
		...
		if (mDefaultLoader == null)
			throw new EasyGEngineRuntimeException(
					"The default safe loader is null");
		SafeLoader loader;
		try {
	...
	private Bridge getBridge(Class<?> firstClass, Class<?> secondClass) {
		return bridgePool.getUsingBridges().get(
