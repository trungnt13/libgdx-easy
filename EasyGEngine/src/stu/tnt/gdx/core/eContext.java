...
import stu.tnt.gdx.assets.StyleLoader;
...
public class eContext {
	...
	public eContext() {
		...
		manager.setLoader(StyleAtlas.class, new StyleLoader(
	...
	public int totalData() {
		int size = 0;
		for (Iterator<Context> iterator = mContextMap.values(); iterator
				.hasNext();) {
	...
	public void removeContext(String contextName) {
		Context tmp = mContextMap.remove(contextName);
	...
	TextureRegion findGRegionByName(String regionName) {
		if (mCurAtlas == null)
			throw new EasyGEngineRuntimeException(
					"Plz use eAdmin.egraphics.atlasQuery(...) first");
	...
	public Pool poolQuery(Class type) {
		...
		if (mCurrentPool == null)
			throw new EasyGEngineRuntimeException(
					"You haven't make any pool for class : " + type.getName());
		return mCurrentPool;
	}
