package stu.tnt.gdx.core.utils;

import stu.tnt.gdx.utils.OnRecycleListener;
...
/**
 * 
 * @FileName: BridgePool.java
 * @CreateOn: Sep 15, 2012 - 11:15:08 AM
 * @Author: TrungNT
 */
public class BridgePool implements OnRecycleListener<Bridge> {
	...
	public Bridge obtain (Class<?> firstClass, Class<?> secondClass) {
		Bridge tmp = freeObjects.size == 0 ? newObject(firstClass, secondClass) : freeObjects.pop().set(firstClass, secondClass);
		this.usingBridge.put(tmp.getName(), tmp);
		return tmp;
	}

	public Bridge obtain (String name) {
		Bridge tmp = freeObjects.size == 0 ? newObject(name) : freeObjects.pop().set(name);
		this.usingBridge.put(tmp.getName(), tmp);
		return tmp;
	}
