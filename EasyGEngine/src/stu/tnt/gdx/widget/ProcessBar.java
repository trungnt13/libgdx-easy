...
public class ProcessBar extends Actor {
	...
	public void increaseProcess(float percent) {
		if (percent < 0 || percent > 1 || mCurrentPercent < 0
				|| mCurrentPercent > 1)
			return;
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if (scaleX == 1 && scaleY == 1 && rotation == 0) {
			batch.draw(mStyle.background, getX(), getY(), getWidth(),
					getHeight());
			...
			batch.draw(mStyle.background, getX(), getY(), getOriginX(),
					getOriginY(), getWidth(), getHeight(), scaleX, scaleY, 90);
			tmp = mStyle.process[mKeyFrame];
			tmp.setRegion((int) tmp.getRegionX(), (int) tmp.getRegionY(),
					(int) (mCurrentPercent * mFirstWidth), (int) mFirstHeight);
			batch.draw(tmp, getX(), getY(), getOriginX(), getOriginY(),
					getWidth(), getHeight(), scaleX, scaleY, 90);
		}
	}
	...
	public void act(float delta) {
		super.act(delta);
		if (isAnimation) {
			if (mStyle.time > 0 && mCurrentTime >= 0
