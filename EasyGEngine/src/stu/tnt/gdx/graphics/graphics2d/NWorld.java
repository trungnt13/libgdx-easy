...
public final class NWorld implements Disposable {
	...
	final LongMap<NativeSpriteBackend> mSpriteMap = new LongMap<NativeSpriteBackend>(100);
	...
	public <T extends NSprite> Pool<T> makeNSpritePool (Class<T> type, Factory<T> factory, int maxSize) {
		final Pool<T> pool = new Pool<T>(maxSize, factory);
		mSpritePoolMap.put(type, pool);
		return pool;
	}

	public <T extends NSpriteA> Pool<T> makeNSpriteAPool (Class<T> type, Factory<T> factory, int maxSize) {
		final Pool<T> pool = new Pool<T>(maxSize, factory);
	...
	public NSprite newSprite () {
		...
		if (sprite.manager == null) mMainManager.manage(sprite);
	...
	void poolSprite (NativeSpriteBackend sprite) {
		if (sprite instanceof NSprite) {
			...
			if (!pool.freeNoReset(sprite)) sprite.dispose();
	...
	/** Direct delete manager 1. check manager = null 2. dispose sprite 3. clear manager list 4. dispose manager {@link NManager} */
	public void delManager (long address) {
		if (address == mMainManager.address) return;
	...
	public void delManager (NManager manager) {
		if (manager == mMainManager) return;
	...
	public void ProcessCollision (NManager manager1, NManager manager2) {
		if (collideListener == null) return;
		processCollision(manager1.address, manager2.address);
	}
	...
	public final native void CollisionConfig (int worldX, int worldY, int worldWidth, int worldHeight, int cols, int row);
