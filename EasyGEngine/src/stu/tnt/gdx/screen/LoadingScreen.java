...
public abstract class LoadingScreen extends Screen implements
		AssetErrorListener {
	...
	public void show() {
		...
		int size = eAdmin.econtext.getQueueAssets()
				+ eAdmin.eaudio.getQueueAssets();
		...
		if (mNewScreen != tmp)
			throw new EasyGEngineRuntimeException(
					"Can't change the next screen at onCreate()");
		if (size != eAdmin.econtext.getQueueAssets()
				+ eAdmin.eaudio.getQueueAssets())
			throw new EasyGEngineRuntimeException(
					"Can't load data at onCreate()");
	...
	public void update(float delta) {
		...
		isDone = isDone & eAdmin.eaudio.update()
				& (eAdmin.eaudio.getQueueAssets() == 0);
